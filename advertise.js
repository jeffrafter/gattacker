...
var options = new getopt([
  ...
  ['S', 'static', 'static - do not connect to ws-server/target device'],
  ['f', 'funmode', 'have fun!'],
...
bleno.on('accept', function (clientAddress) {
  ...
  if (clientAddress === myAddress) {
    ...
    if (!staticRun) {
      //notify the ws-server of victim's connection
...
//update the ws-server on client disconnect
...
wsclient.on('notification', function (peripheralId, serviceUuid, uuid, data) {
  ...
  if (hook) {
    ...
    if (!subscriptions[serviceUuid]) {
      //the ws-server received notification from previous connection
